...
/// Allow-listed trust boundary: a single bad field rejects the entire document,
/// falling back to the built-in catalogue.
/// </summary>
public static class ProviderCatalogue
{
    public const int SupportedSchema = 1;
    public const int MaxProviders = 200;
    ...
    public static bool TryParse(string? json, out List<ProviderItem>? providers, out string? error)
    {
        providers = null;
        error = null;
        try
        {
            ...
            foreach (JToken entry in entries)
            {
                ...
                if (provider == null) return Fail("Failed to parse provider entry.", out error);
